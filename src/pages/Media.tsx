...
export default function Media(props: Props) {
  ...
  function exportPostBuyReport() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>PostBuy 分析报告</title><style>body{font-family:'PingFang SC','Microsoft YaHei',sans-serif;max-width:900px;margin:40px auto}h1{font-size:22px}h1 span{color:#FE2C55}table{width:100%;border-collapse:collapse;font-size:13px;margin-top:16px}th,td{border:1px solid #E4E6EB;padding:8px 10px}th{background:#F5F6F8}</style></head><body><h1>PostBuy 分析报告 <span>· ${new Date().toLocaleDateString("zh-CN")}</span></h1><p>共 ${postbuys.length} 条 · 平均 ROI 1:${avgRoi}</p><table><tr><th>资源</th><th>月份</th><th>曝光</th><th>CTR</th><th>点击</th><th>CPM</th><th>ROI</th><th>数据来源</th></tr>${rows}</table></body></html>`;
  ...
  function exportQuoteHtml() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>自助报价单</title><style>body{font-family:'PingFang SC','Microsoft YaHei',sans-serif;max-width:760px;margin:40px auto}h1{font-size:22px}h1 span{color:#FE2C55}table{width:100%;border-collapse:collapse;font-size:13px;margin-top:16px}th,td{border:1px solid #E4E6EB;padding:8px 10px}th{background:#F5F6F8}.total{margin-top:12px;font-size:16px;font-weight:700}</style></head><body><h1>自助报价单 <span>· ${nameOf(qCustomer)}</span></h1><table><tr><th>#</th><th>媒体资源</th><th>点位</th><th>时长</th><th>单价</th><th>小计</th></tr>${rows}</table><p class="total">合计:${money(quoteTotal)}</p></body></html>`;
    const blob = new Blob([html], { type: "text/html" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
  ...
  function exportScheduleQuote() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>排期报价单</title><style>body{font-family:'PingFang SC','Microsoft YaHei',sans-serif;max-width:760px;margin:40px auto}table{width:100%;border-collapse:collapse;font-size:13px;margin-top:16px}th,td{border:1px solid #E4E6EB;padding:8px 10px}th{background:#F5F6F8}.total{margin-top:12px;font-size:16px;font-weight:700}</style></head><body><h1>媒介排期报价单</h1><table><tr><th>项目</th><th>媒体</th><th>窗口</th><th>报价</th></tr>${rows}</table><p class="total">合计:${money(sell)}</p></body></html>`;
    const blob = new Blob([html], { type: "text/html" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
